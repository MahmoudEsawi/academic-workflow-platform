...
import { GraduationCap, Mail, Lock, Sparkles, ArrowRight, ArrowLeft, CheckCircle2, UserCheck, Shield } from 'lucide-react';
...
const Login = () => {
    ...
    useEffect(() => {
        if (roleParam === 'Supervisor') {
            fillCredentials('smith@university.edu');
        } else if (roleParam === 'Admin') {
            fillCredentials('admin@university.edu');
        } else if (roleParam === 'Student') {
            fillCredentials('alice@student.edu');
        }
    }, [roleParam]);
            ...
            <div className="hidden lg:flex lg:w-1/2 bg-[#1E3A2F] text-white flex-col justify-between p-12 relative overflow-hidden">
                ...
                <div className="relative z-10 max-w-md my-auto space-y-6">
                    <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/10 text-white text-xs font-bold">
                        <Shield size={14} /> Secure Role-Based Access
                    </div>
                    <h1 className="text-4xl sm:text-5xl font-extrabold text-white tracking-tight leading-tight">
                        Turn Ambition <br />
                        into <span className="font-serif-editorial italic font-normal text-[#A3CFBB]">Achievement</span>
                    </h1>
                    <p className="text-white/70 text-sm leading-relaxed">
                        Sign in to access your graduation research dashboard, sprint deliverables, supervisor evaluations, and team chat.
                    </p>
                    ...
                    <div className="space-y-3 pt-2">
                        <div className="flex items-center gap-3 text-xs text-white/90">
                            ...
                            <span>Protected by HttpOnly JWT sessions & role middleware</span>
                        </div>
                        <div className="flex items-center gap-3 text-xs text-white/90">
                            <div className="w-5 h-5 rounded-full bg-white/10 text-[#A3CFBB] flex items-center justify-center">
                                <CheckCircle2 size={13} />
                            </div>
                            <span>Instant redirection to student/supervisor dashboard</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="max-w-md w-full mx-auto space-y-6">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-[#596F65]">
                            Enter your university credentials to enter the workspace.
                        </p>
                    </div>

                    {/* Quick Demo Fillers */}
                    <div className="p-4 rounded-2xl bg-[#FAF9F5] border border-[#E5ECE8]">
                        <p className="text-xs font-bold text-[#1E3A2F] mb-2 flex items-center gap-1.5">
                            <UserCheck size={14} className="text-[#3E735E]" />
                            1-Click Demo Gateways:
                        </p>
                        <div className="grid grid-cols-3 gap-2">
                            <button
                                type="button"
                                onClick={() => fillCredentials('alice@student.edu')}
                                className="px-2.5 py-2 rounded-xl bg-white hover:bg-[#1E3A2F] hover:text-white border border-[#D5DDD8] text-[#1E3A2F] text-xs font-bold transition-all text-center shadow-sm"
                            >
                                🎓 Student
                            </button>
                            <button
                                type="button"
                                onClick={() => fillCredentials('smith@university.edu')}
                                className="px-2.5 py-2 rounded-xl bg-white hover:bg-[#1E3A2F] hover:text-white border border-[#D5DDD8] text-[#1E3A2F] text-xs font-bold transition-all text-center shadow-sm"
                            >
                                👨‍🏫 Doctor
                            </button>
                    ...
                    <form className="space-y-4" onSubmit={handleLogin}>
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full pl-10 pr-4 py-3 bg-[#FAF9F5] border border-[#D5DDD8] rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#1E3A2F]/20 focus:border-[#1E3A2F] text-xs text-[#1A2421] placeholder-slate-400 transition-all"
                                    value={email}
                        ...
                        <button
                            type="submit"
                            disabled={isLoading}
                            className="w-full py-3.5 px-4 bg-[#1E3A2F] hover:bg-[#142820] text-white font-bold rounded-full shadow-lg shadow-[#1E3A2F]/20 transition-all hover:scale-[1.01] active:scale-[0.99] text-xs flex items-center justify-center gap-2 disabled:opacity-50"
                        >
                    ...
                    <p className="text-center text-xs text-[#596F65] pt-2">
                        ...
                        <Link to="/signup" className="font-bold text-[#1E3A2F] hover:underline">
